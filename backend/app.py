...
# Connect to MongoDB
client = MongoClient('mongodb+srv://UserTest:UserTest123@usertest.am1ig.mongodb.net/?retryWrites=true&w=majority&appName=UserTest')
...
def skill_analyze():
    if 'resume' not in request.files or 'job_description' not in request.files:
        return jsonify({'message': 'Both resume and job description are required'}), 400

    resume_file = request.files['resume']
    job_description_file = request.files['job_description']

    if resume_file.filename == '' or job_description_file.filename == '':
        return jsonify({'message': 'Both files must be provided'}), 400

    try:
        # Extract text from resume PDF
        resume_text = extract_text_from_pdf(resume_file)
        print("Resume Text:")
        print(resume_text)  # Print the resume text to the command line

        # Extract text from job description PDF
        job_description_text = extract_text_from_pdf(job_description_file)
        print("Job Description Text:")
        print(job_description_text)  # Print the job description text to the command line
